...
def topo_sort(workflow):
    ...
    #         if len(zoreInDegree) == 1:
    #             func = zoreInDegree.pop()
    #         elif 'min' in candidate:
    ...
    while (not (not zoreInDegree)):
        if flag:
            for key, candidate in enumerate(zoreInDegree):
                if 'min' not in candidate:
                    func = zoreInDegree.pop(key)
                    flag = 0
                    break
            if flag:
                func = zoreInDegree.pop(0)
        else:
            func = zoreInDegree.pop(0)
        topo.append(func)
        for next_node_name in workflow.nodes[func].next:
...
def save_grouping_config(workflow: component.workflow, node_info, info_dict, info_raw_dict, node_ip):
    repo = repository.Repository(workflow.workflow_name, node_ip)
...
def find_min_group(name: str, group_dict: dict):
    _,func_stage_idx,func_group_idx = name.split('-')
    func_stage_idx = int(func_stage_idx)
    func_group_idx = int(func_group_idx)
    try:
        group_dict['bundlingstage-{}'.format(func_stage_idx//2)][func_group_idx].append(name)
    except:
        if 'bundlingstage-{}'.format(func_stage_idx//2) not in group_dict:
            group_dict['bundlingstage-{}'.format(func_stage_idx//2)] = {func_group_idx: [name]}
        else:
            group_dict['bundlingstage-{}'.format(func_stage_idx//2)][func_group_idx] = [name]
...
def get_grouping_config(workflow: component.workflow, node_info_dict):
    ...
    if w1.nodes:
        ...
        for node in w1.nodes:
            find_min_group(node, group_dict)
    ...
    if w2.nodes:
        ...
        for k,v in enumerate(list(w2.nodes.keys())): # w2 is foreach
            w1_group_detail.append(tuple([v]))
            group_ip[tuple([v])] = node_info_list[k % node_number]
    ...
    for node_name in workflow.nodes:
        ...
        function_info_raw = {'function_name': node.name, 'runtime': node.runtime, 'to': 'DB', 'ip': ip,
                             'parent_cnt': workflow.parent_cnt[node.name], 'conditions': node.conditions}
        function_input = dict()
...
if __name__ == '__main__':
    ...
    node_ip = sys.argv[2]
    workflow_pool = sys.argv[3:]
    
    for workflow_name in workflow_pool:
        config_path = config.FUNCTION_INFO_ADDRS
        for wfname, addr in config_path.items():
            parse_yaml_min.gen_workflow(addr, split_ratio)
        workflow = parse_yaml_min.parse(workflow_name)
