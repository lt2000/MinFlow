...
import config
from parse_yaml_bundling import test
...
class WorkerSPManager:
    def __init__(self, workflow_name: str, data_mode: str):
        global min_port
    ...
    def gen_io_keys(self, func_name, info, func_type):
        ...
        if func_type == self.ft.NORMAL:  # func is normal
            ...
            keys = {1: {'input': input_keys, 'output': output_keys}}
            ...
            first_phase = {'input': new_input, 'output': new_output}
            ...
            second_phase = {'input': new_input, 'output': new_output}
    ...
    def run_bundling(self, info: Any) -> None:
    ...
    def run_foreach(self, info: Any) -> None:
    ...
    def run_min(self, info: Any) -> None:
        input_keys, output_keys = self.gen_io_keys(
    ...
    def run_normal(self, info: Any) -> None:
...
if __name__ == "__main__":
    # funclist = list(workflow.nodes.keys())
    funclist = ['minpart-0-0']
    worker = WorkerSPManager('mapreduce-sort', 'optimized')
    for func in funclist:
        info = worker.get_function_info(func)
        if func in worker.bundling_func:
            worker.run_bundling(info)
        elif func in worker.min_func:
            worker.run_min(info)
        elif func in worker.foreach_func:
            worker.run_foreach(info)
        else:
            worker.run_normal(info)
