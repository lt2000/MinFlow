...
app = Flask(__name__)
docker_client = docker.from_env(max_pool_size=100)
container_names = []
...
def resource():
    ...
    req_id = data['req_id']
    print(node)
    r = subprocess.Popen(["python", "resource.py", str(node), req_id])
    redis_db = redis.StrictRedis(host=config.REDIS_HOST, port=config.REDIS_PORT, db=2)
    redis_db.set('pid', r.pid)
...
def clear():
    if config.BALANCE_STATISTICS:
        redis_db = redis.StrictRedis(host=config.REDIS_HOST, port=config.REDIS_PORT, db=2)
        pid = int(redis_db.get('pid').decode())
