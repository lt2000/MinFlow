...
def run_workflow(workflow_name, request_id):
    ...
    if config.BALANCE_STATISTICS:
        addrs = repo.get_all_addrs(workflow_name + '_workflow_metadata')
        jobs = []
        for addr in addrs:
            node_url = f'http://{addr}/resource'
            data = {'node': addr.split(':')[0], 'req_id': request_id}
            jobs.append(gevent.spawn(lambda u, d: requests.post(u, json=d), node_url, data))
        gevent.joinall(jobs)
