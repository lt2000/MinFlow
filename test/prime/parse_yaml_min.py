...
def print_workflow(nodes):
    for name in nodes:
        ...
        print('function split_ratio: ', nodes[name].split_ratio)
...
def parse(workflow_name):
    with open('alpha_3.pickle', 'rb') as file:
    ...
    for function in functions:
        ...
        if 'next' in function:
            ...
            if function['next']['type'] == 'switch':
                ...
                new_n = function['next']['split_ratio']
                if shuffle_mode == 'min':   
                    try: 
                        new_n = alpha_3[function['next']['split_ratio']]      
                    except:
                        pass
                              
                net = network.min_generator(new_n)
                if shuffle_mode == 'min':
                ...
                if name in shuffle_functions:   # current function is a mapper
                    ...
                    if shuffle_mode == 'min':
                        ...
                        temp_foreach_num = len(net.foreach_size)
                ...
                for j in range(temp_foreach_num):
                    ...
                    if 'split_ratio' in function:
                        if shuffle_mode == 'min':
                            ...
                            current_function.set_split_ratio(
                            net.foreach_size[j])  # set split ratio
                    total = total + 1
                    nodes[name] = current_function
            continue
