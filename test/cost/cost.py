...
class min_generator:
    ...
    def min_optimized(self):
        ...
        if self.modeler:
            ...
            self.opt = copy.deepcopy(opt_net)  
...
def cal(n):
    ...
    modeler = True
    net = min_generator(n,modeler)
    n4 = 2*n + 2*min(net.opt)*n
    
    return  [n1/n1,n2/n1,n3/n1,n4/n1]
...
x = ['Baseline','FaaSFlow','Lambada','MinFlow']
y = cal(func_nums)
ax1.plot(x, y, color='#D62026', marker='^',markerfacecolor='none') 

ax1.set_ylabel('Normalized storage cost', fontsize=12)
